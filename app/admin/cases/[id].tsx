...
/** List route. Do not use `/admin/cases/index` — it is captured by `[id]` as id "index". */
const USAGE_CASES_LIST_HREF = '/admin/cases' as unknown as Href;
...
function normalizeRagMatches(raw: unknown): RagMatch[] {
  if (!Array.isArray(raw)) return [];
  return raw.filter((r): r is RagMatch => !!r && typeof r === 'object');
}
...
function formatRagMatchLabel(match: RagMatch): string {
  const rule = typeof match.rule_number === 'string' ? match.rule_number : '';
  const tags: string[] = [];
  if (match.anchored) tags.push('anchored');
  if (match.expanded) tags.push('expanded');
...
export default function UsageCaseDetailScreen() {
  ...
  const [row, setRow] = useState<UsageCaseRow | null>(null);
  ...
  const rulesList = useMemo(
    () => (row ? normalizeRulesCited(row.rules_cited) : []),
    [row],
  );

  const ragMatches = useMemo(
    () => (row ? normalizeRagMatches(row.rag_matches) : []),
    [row],
  );
    ...
    async function load() {
      ...
      try {
        const res = await fetch(
          `${BACKEND_BASE_URL}/admin/cases/${encodeURIComponent(caseId)}`,
        );
        if (res.status === 404) {
        ...
        const json = (await res.json()) as UsageCaseDetailResponse;
        const c = json.case;
        if (cancelled) return;
    ...
    <View style={styles.root}>
      ...
      >
        <Pressable
          ...
          accessibilityLabel="Back to cases list"
          onPress={() => router.replace(USAGE_CASES_LIST_HREF)}
          style={({ pressed }) => [styles.adminBackLink, pressed && styles.pressed]}
        >
          <Text style={styles.adminBackLinkText}>← Cases</Text>
        </Pressable>
        ...
        {!loading && row && !notFound && !errorMessage ? (
          <>
            ...
            {row.situation?.trim() || row.selected_category?.trim() ? (
              <View style={[styles.section, styles.contextSection]}>
                {row.situation?.trim() ? (
                  <View style={styles.situationBlockFirst}>
                    <Text style={[styles.sectionLabel, { marginTop: 0 }]}>
            ...
            {ragMatches.length > 0 ? (
              <View style={[styles.section, { paddingBottom: 0 }]}>
                ...
                <View style={styles.rulesRow}>
                  {ragMatches.map((match, i) => (
                    <View key={`rag-${i}`} style={styles.ruleTag}>
                      <Text style={styles.ruleTagText} numberOfLines={2}>
                        {formatRagMatchLabel(match)}
                      </Text>
                    </View>
                  ))}
                </View>
            ...
            <View style={[styles.section, styles.contextSection]}>
              ...
              {row.cr_version?.trim() ? (
                <View style={styles.situationBlock}>
                  ...
                  <Text style={styles.situationText}>{row.cr_version.trim()}</Text>
                </View>
              ) : null}
              {row.case_id?.trim() ? (
                <View style={styles.situationBlock}>
                  ...
                  <Text style={styles.situationText}>{row.case_id.trim()}</Text>
                </View>
              ) : null}
              {row.session_id?.trim() ? (
                <View style={styles.situationBlock}>
                  ...
                  <Text style={styles.situationText}>{row.session_id.trim()}</Text>
                </View>
              ) : null}
              {row.ip_address?.trim() ? (
                <View style={styles.situationBlock}>
                  ...
                  <Text style={styles.situationText}>{row.ip_address.trim()}</Text>
                </View>
              ) : null}
            </View>
